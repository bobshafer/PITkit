...
# --- MAIN EXECUTION (Modified for Box Constraints) ---
...
function main()
    ...
    # --- NEW: Define Physical Boundaries ---
    # We will set boundaries to stop the optimizer from "cheating"
    # [rho_0, r_0, Upsilon_disk, sigma_int]
    lower_bounds_phys = [1e-6, 0.1, 0.1, 0.1]
    upper_bounds_phys = [1.0,  50.0, 2.0, 50.0]
    
    # --- MODIFICATION: Initial guesses for 4 free parameters ---
    initial_guesses_phys = [0.01, 10.0, 0.5, 1.0] 
    println("--- RUNNING BOX-CONSTRAINED FITTER (Burkert Profile) ---")
    println("Testing Corrected Hypothesis (v_total^2 = v_baryon^2 + v_halo^2):")
    println("Optimizing 4 free parameters with BOUNDARIES:")
    println("  rho_0: [$(lower_bounds_phys[1]), $(upper_bounds_phys[1])]")
    println("  r_0:   [$(lower_bounds_phys[2]), $(upper_bounds_phys[2])]")
    println("  Upsilon: [$(lower_bounds_phys[3]), $(upper_bounds_phys[3])]")
    println("  sigma_int: [$(lower_bounds_phys[4]), $(upper_bounds_phys[4])]")
    println("Initial physical guess: ", initial_guesses_phys)
    # --- END MODIFICATION ---

    # --- NEW: Convert bounds to unconstrained space for Fminbox ---
    # Fminbox requires the unconstrained (log-like) bounds
    ...
    # Ensure initial guess is within bounds (it should be)
    ...
    try
        ...
        # --- NEW: Use Fminbox with LBFGS ---
        # This uses the LBFGS algorithm but enforces our bounds
    ...
    println("\n--- Fit Results (BOX-CONSTRAINED FIT) ---");
