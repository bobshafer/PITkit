# pit_constrained_fitter.jl
#
# This is a MODIFIED version of the global fitter.
# It tests the "Constrained Hypothesis" from the PIT simulator.
#
# LOCKED PARAMETERS (from theory/simulation):
#   mu = 0.005
#   kc = 0.02
#   p  = 4.0
#
# FREE PARAMETERS (to be fit):
#   K0, Upsilon_disk, sigma_int
#
# --- Dependencies ---
using Printf
using Statistics
...
function transform_params(params_unconstrained::Vector{Float64})
    # This now only transforms the 3 free parameters
...
function predict_galaxy_rotation_curve(
    params_phys_3::Vector{Float64}, # K0, Upsilon_disk
    r::Vector{Float64}, 
    v_gas_sq::Vector{Float64}, 
    v_disk_sq::Vector{Float64}
)
    # --- MODIFICATION: Set 3 parameters from our hypothesis ---
    # (Removed 'const' to fix the syntax error)
    kc = 0.5       # Predicted from simulator
    p  = 4.0        # Predicted from simulator (super-sharp)
    mu_param = 0.005  # Goldilocks "engine"
    # --- END MODIFICATION ---

    # Get the 2 free parameters for this function
    K0 = max(params_phys_3[1], 1e-9)
    Upsilon_disk = max(params_phys_3[2], 1e-3)
    ...
    # --- MODIFICATION: W_k calculation now uses our *fixed* kc and p ---
    k_ratio = k_grid ./ kc  # kc is now our constant 0.02
    k_ratio_safe = max.(k_ratio, 0.0)
    exponent_val = map(x -> x^p, k_ratio_safe .+ 1e-15) # p is now our constant 4.0
    # --- END MODIFICATION ---
    
    exponent_val_clamped = min.(exponent_val, 100.0)
    W_k = K0 .* exp.(-exponent_val_clamped) # K0 is now our 1st free param
...
function objective_function(params_unconstrained::Vector{Float64}, all_galaxy_data::Vector{GalaxyData})
     ...
     # --- MODIFICATION: We now only optimize 3 parameters ---
     params_phys = transform_params(params_unconstrained)
     if length(params_phys) != 3 # K0, Upsilon_disk, sigma_int
        return LARGE_FINITE_VAL * 1000.0
     end
     
     sigma_int = max(params_phys[3], 1e-9) # 3rd param is sigma_int
     params_phys_2 = params_phys[1:2]      # 1st/2nd are K0, Upsilon
     # --- END MODIFICATION ---
     ...
     for galaxy in all_galaxy_data
         ...
         # --- MODIFICATION: Pass the 2 free params to the prediction function ---
         v_model_sq = predict_galaxy_rotation_curve(params_phys_2, r, v_gas_sq, v_disk_sq)
         # --- END MODIFICATION ---
...
function main()
    ...
    # --- MODIFICATION: Initial guesses for only 3 free parameters ---
    # K0, Upsilon_disk, sigma_int
    initial_guesses_phys = [1.0, 0.5, 1.0] 
    println("--- RUNNING CONSTRAINED FITTER ---")
    println("Testing Hypothesis:")
    println("  mu (fixed) = 0.005")
    println("  kc (fixed) = 0.5")
    println("  p  (fixed) = 4.0")
    println("Optimizing 3 free parameters: K0, Upsilon_disk, sigma_int")
    ...
    # --- MODIFICATION: Number of parameters is now 3 ---
    num_parameters = 3
    # --- END MODIFICATION ---
    ...
    println("\n--- Fit Results (CONSTRAINED FIT) ---");
    # --- MODIFICATION: Update param_names list ---
    param_names = ["K0 (free)", "Upsilon_disk (free)", "sigma_int (free)"]
    param_names_fixed = ["mu (fixed)", "kc (fixed)", "p (fixed)"]
    params_fixed = [0.005, 0.5, 4.0]
    # --- END MODIFICATION ---
