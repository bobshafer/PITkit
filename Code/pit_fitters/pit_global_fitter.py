...
def predict_galaxy_rotation_curve(params_phys, galaxy_data):
    ...
    # --- FIX: Aggressive final clamp ---
    v_model_sq = jnp.maximum(v_model_sq, 1e-9) # Keep lower bound
    v_model_sq = jnp.nan_to_num(v_model_sq, nan=1e6, posinf=1e6, neginf=0.0) # Replace bad values with upper bound
    v_model_sq = jnp.minimum(v_model_sq, 1e6) # Enforce upper bound (e.g., (1000 km/s)^2)
    # --- END FIX ---
...
def galaxy_loss_internal(params_unconstrained, galaxy_data, mask):
    ...
    # v_model_sq already clamped in predict function
    ...
    # No need to clamp squared_diff if v_model_sq is clamped reasonably
...
# (load_galaxy_data, create_dummy_data, pad_and_stack_data remain the same)
...
def run_optimization(initial_params_unconstrained, all_galaxy_data, all_masks, maxiter=500):
    ...
    optimizer = jaxopt.LBFGS(fun=loss_fn_for_opt, maxiter=maxiter, tol=1e-5,
                             ...
                             implicit_diff=False, stepsize=-1.0) # Use backtracking linesearch
    ...
    if hasattr(state, 'error') and state.error is not None:
         if np.isnan(state.error): print("Warning: Optimization failed - LBFGS error is NaN.")
         elif state.error > optimizer.tol: print(f"Warning: Opt might not have converged. Final LBFGS error: {state.error:.2e} > tol {optimizer.tol:.1e}")
         else: print(f"Optimization converged. Final LBFGS error: {state.error:.2e}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        if jnp.isnan(init_loss) or jnp.isinf(init_loss) or init_loss >= LARGE_FINITE_VAL:
             print(f"Warning: Initial loss is {init_loss}. Parameters may be in unstable region.")
             # Decide whether to abort or continue
             # exit(1) # Abort if initial state is clearly bad
    except Exception as e:
